...
class ObstacleAlignmentReport {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.header = null;
      this.id = null;
      this.ObstacleAlignement = null;
      ...
      if (initObj.hasOwnProperty('ObstacleAlignement')) {
        this.ObstacleAlignement = initObj.ObstacleAlignement
      }
      else {
        this.ObstacleAlignement = new Array(127).fill(new ObstacleAlignment());
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Check that the constant length array field [ObstacleAlignement] has the right length
    if (obj.ObstacleAlignement.length !== 127) {
      throw new Error('Unable to serialize array field ObstacleAlignement - length must be 127')
    }
    // Serialize message field [ObstacleAlignement]
    obj.ObstacleAlignement.forEach((val) => {
      bufferOffset = ObstacleAlignment.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new ObstacleAlignmentReport(null);
    // Deserialize message field [header]
    data.header = std_msgs.msg.Header.deserialize(buffer, bufferOffset);
    // Deserialize message field [id]
    data.id = std_msgs.msg.UInt8.deserialize(buffer, bufferOffset);
    // Deserialize message field [ObstacleAlignement]
    len = 127;
    data.ObstacleAlignement = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.ObstacleAlignement[i] = ObstacleAlignment.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    object.ObstacleAlignement.forEach((val) => {
      length += ObstacleAlignment.getMessageSize(val);
    });
  ...
  static Resolve(msg) {
    ...
    const resolved = new ObstacleAlignmentReport(null);
    ...
    if (msg.ObstacleAlignement !== undefined) {
      resolved.ObstacleAlignement = new Array(127)
      for (let i = 0; i < resolved.ObstacleAlignement.length; ++i) {
        if (msg.ObstacleAlignement.length > i) {
          resolved.ObstacleAlignement[i] = ObstacleAlignment.Resolve(msg.ObstacleAlignement[i]);
        }
        else {
          resolved.ObstacleAlignement[i] = new ObstacleAlignment();
        }
      }
    }
    else {
      resolved.ObstacleAlignement = new Array(127).fill(new ObstacleAlignment())
    }
...
ObstacleAlignmentReport.Constants = {
  OBSTACLE_ALIGNMENT_REPORT_SIZE: 127,
}

module.exports = ObstacleAlignmentReport;
