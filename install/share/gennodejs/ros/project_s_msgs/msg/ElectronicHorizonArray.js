...
class ElectronicHorizonArray {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('PathsArray')) {
        this.PathsArray = initObj.PathsArray
      }
      else {
        this.PathsArray = new Array(5).fill(new ElectronicHorizon());
      }
      if (initObj.hasOwnProperty('NbrPath')) {
        this.NbrPath = initObj.NbrPath
      }
      else {
        this.NbrPath = new std_msgs.msg.UInt8();
      }
      if (initObj.hasOwnProperty('indexMPP')) {
        this.indexMPP = initObj.indexMPP
      }
      else {
        this.indexMPP = new std_msgs.msg.UInt8();
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Check that the constant length array field [PathsArray] has the right length
    if (obj.PathsArray.length !== 5) {
      throw new Error('Unable to serialize array field PathsArray - length must be 5')
    }
    // Serialize message field [PathsArray]
    obj.PathsArray.forEach((val) => {
      bufferOffset = ElectronicHorizon.serialize(val, buffer, bufferOffset);
    });
    // Serialize message field [NbrPath]
    bufferOffset = std_msgs.msg.UInt8.serialize(obj.NbrPath, buffer, bufferOffset);
    // Serialize message field [indexMPP]
    bufferOffset = std_msgs.msg.UInt8.serialize(obj.indexMPP, buffer, bufferOffset);
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new ElectronicHorizonArray(null);
    // Deserialize message field [header]
    data.header = std_msgs.msg.Header.deserialize(buffer, bufferOffset);
    // Deserialize message field [id]
    data.id = std_msgs.msg.UInt8.deserialize(buffer, bufferOffset);
    // Deserialize message field [PathsArray]
    len = 5;
    data.PathsArray = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.PathsArray[i] = ElectronicHorizon.deserialize(buffer, bufferOffset)
    }
    // Deserialize message field [NbrPath]
    data.NbrPath = std_msgs.msg.UInt8.deserialize(buffer, bufferOffset);
    // Deserialize message field [indexMPP]
    data.indexMPP = std_msgs.msg.UInt8.deserialize(buffer, bufferOffset);
  ...
  static getMessageSize(object) {
    ...
    object.PathsArray.forEach((val) => {
      length += ElectronicHorizon.getMessageSize(val);
    });
  ...
  static datatype() {
    ...
    return 'project_s_msgs/ElectronicHorizonArray';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '1944698f685c5272c5b7372763d0c8f3';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new ElectronicHorizonArray(null);
    ...
    if (msg.PathsArray !== undefined) {
      resolved.PathsArray = new Array(5)
      for (let i = 0; i < resolved.PathsArray.length; ++i) {
        if (msg.PathsArray.length > i) {
          resolved.PathsArray[i] = ElectronicHorizon.Resolve(msg.PathsArray[i]);
        }
        else {
          resolved.PathsArray[i] = new ElectronicHorizon();
        }
      }
    }
    else {
      resolved.PathsArray = new Array(5).fill(new ElectronicHorizon())
    }

    if (msg.NbrPath !== undefined) {
      resolved.NbrPath = std_msgs.msg.UInt8.Resolve(msg.NbrPath)
    }
    else {
      resolved.NbrPath = new std_msgs.msg.UInt8()
    }

    if (msg.indexMPP !== undefined) {
      resolved.indexMPP = std_msgs.msg.UInt8.Resolve(msg.indexMPP)
    }
    else {
      resolved.indexMPP = new std_msgs.msg.UInt8()
    }
...
module.exports = ElectronicHorizonArray;
