...
while(tempo){
 tempo--;
										...
										circuit_manager_cmd.data = CIRCUIT_GO;
  										pub_circuit_manager_cmd.publish(circuit_manager_cmd);
#else
										debug_msg.data = "circuit_manager_cmd.data = CIRCUIT_GO";
										pub_mission_manager_debug.publish(debug_msg);										
#endif
									}
									break;
...
float delta_offset;
delta_offset = roadModel_EH.electronicHorizon.ehPosition.offset.data - 0;

float vitesse_effective;
vitesse_effective = sqrt(pow(ego_data_report.EgoData[0].velocity.linear.x,2) + pow(ego_data_report.EgoData[0].velocity.linear.y,2));
// FIN
							...
							switch (fromMissionServer.stateCmd) {
								...
								case project_s_msgs::fromMissionServer::WAITING:
									...
									circuit_manager_cmd.data = CIRCUIT_STOP;
  									pub_circuit_manager_cmd.publish(circuit_manager_cmd);
#else
									debug_msg.data = "circuit_manager_cmd.data = CIRCUIT_STOP";
									pub_mission_manager_debug.publish(debug_msg);										
#endif
