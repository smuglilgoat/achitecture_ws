...
/*
			{
			// Amers Text
			visualization_msgs::Marker AmerMarkerText;
			AmerMarkerText.header.frame_id = "/base_link";
			AmerMarkerText.header.stamp = ros::Time::now();
			AmerMarkerText.lifetime = ros::Duration(0.2);
			AmerMarkerText.id = 533;
			AmerMarkerText.ns = "Electronic_Horizon";
			AmerMarkerText.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
			AmerMarkerText.scale.x = 0.5;
			AmerMarkerText.scale.y = 0.5;
			AmerMarkerText.scale.z = 0.5;
			AmerMarkerText.color.r = 1.0f;
			AmerMarkerText.color.g = 1.0f;
			AmerMarkerText.color.b = 1.0f;
			AmerMarkerText.color.a = 1.0;
			AmerMarkerText.action = visualization_msgs::Marker::MODIFY;
			AmerMarkerText.pose.orientation.w = 1;
			AmerMarkerText.text = "Amer";

			for(uint j = 0; j < roadModel_EH.nbrAmer.data; j++){
				AmerMarkerText.points.push_back(roadModel_EH.amer[j]);
			}

			Lines_EH_Array.markers.push_back(AmerMarkerText);
			}
*/
...
//------
			...
			{	
				...
				AmerMarker.type = visualization_msgs::Marker::CYLINDER;
				AmerMarker.scale.x = 0.2;
				AmerMarker.scale.y = 0.2;
				AmerMarker.scale.z = 1.0;
			...
			{	// ME ligne de droite
				...
				for(uint j = 0; j < roadModel_EH.roadLine[4].QtySegment.data; j++){
					marker.points.push_back(roadModel_EH.roadLine[4].segment[j].position);
				}
			...
			{	// ME ligne de gauche
				...
				for(uint j = 0; j < roadModel_EH.roadLine[5].QtySegment.data; j++){
					marker.points.push_back(roadModel_EH.roadLine[5].segment[j].position);
				}
