...
int main(int argc, char *argv[])
{
	...
	ros::init(argc, argv, std::string("obstacles_markers_rardar_ESR"));

	ros::NodeHandle n;

	ros::Publisher pub_obstacle_rel = n.advertise<visualization_msgs::Marker>(std::string("/radar_ESR_relative_obstacle_markers"), 1);
	ros::Publisher pub_obstacle_abs = n.advertise<visualization_msgs::Marker>(std::string("/radar_ESR_absolu_obstacle_markers"), 1);

	ros::Subscriber subObstaclesAlign = n.subscribe(std::string("/radar_ESR_obstacles_align_data"), 1, obstacleAlignCallback);
	...
	while(n.ok())
	{
		...
		if (received)
		{
			...
			relativeObstacleMarker.color.b = 1.0f;
