...
{
	ros::init(argc, argv, "camera_sekonix_data_extraction");

	ros::NodeHandle n;

	// Publisher
	ros::Publisher pub_obstacles   = n.advertise<project_s_msgs::ObstacleReport>(std::string("/camera_sekonix_data_extraction_obstacle"), 1);	
	ros::Publisher pub_route_lines = n.advertise<project_s_msgs::RouteLine>     (std::string("/camera_sekonix_data_extraction_route_lines"), 1);

	// Subscriber
	ros::Subscriber subData  = n.subscribe(std::string("/camera_sekonix_data_merger_obstacle"),    1, obstacleDataCallback);
	ros::Subscriber subLines = n.subscribe(std::string("/camera_sekonix_data_merger_route_lines"), 1, linesDataCallback);
