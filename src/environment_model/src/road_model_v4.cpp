	...
	while(n.ok())
	{
		...
		if(electronicHorizonArrayReceived)
		{
			...
			{ int i_veh=0;
				...
				{	
					for(uint i=0; i<roadModelTrajBased.roadLine[3].QtySegment.data-1; i++)
					{
						...
						//roadModel_EH.roadLine[4].segment[m].position.y += latDist[i_veh];
					}
				}
			}
			//
			...
			// added by Abbas
				...
				{ int i_veh=0;
					...
					{	
						...
						//	roadModel_EH.roadLine[5].segment[m].position.y += latDist[i_veh];
						}
					}
				}
				...
				for(int id = 0; id < obstacleFusion.QtyObstacles.data; id++)
				{
						...
						|| (obstacleFusion.ObstacleFusion[id].existenceState.data == project_s_msgs::ObstacleFusion::POST_TRACK)))
					{
						...
						if(index < project_s_msgs::RoadModel::SPEED_LIMIT_ARRAY_SIZE)
						{
							...
							if(delta_offset < 1.5*Distance_darret_obligatoire && obstacleFusion.ObstacleFusion[id].obstaclePosition.position.y > -0.5){  
								speedCollisionAvoidance[0].offset = 0;
