...
int main(int argc, char *argv[])
{
	...
	// added by Abbas
	ros::Subscriber trajectory_GPS_pub	= n.subscribe(std::string("/trajectory_GPS_generation_data"),        1, trajectoryCallback);
	//
...
//if (read_EH == false) {
			...
			/*
			// added by Abbas
				double latDist[roadModel.roadLine[3].QtySegment.data];
				...
				if(receivedTrajectory_GPS)
				{
					for(uint i=0; i<roadModel.roadLine[3].QtySegment.data; i++)
					{
							latDist[i] =abs(trajectory_GPS.trajectoryLine.segment[i].position.y 
											- roadModel.roadLine[3].segment[i].position.y);
							if (latDist[i] != 0)
							{
								roadModel.roadLine[3].segment[i].position.x = trajectory_GPS.trajectoryLine.segment[i].position.x;
								roadModel.roadLine[3].segment[i].position.y += latDist[i];
...
// added by Abbas
	if(receivedTrajectory_GPS)
	{
		...
		{	
			for(uint i=0; i<roadModel.roadLine[3].QtySegment.data-1; i++)
			{
				if(roadModel.roadLine[3].segment[i].offset <= roadModel_EH.roadLine[4].segment[m].offset 
					&& roadModel_EH.roadLine[4].segment[m].offset <= roadModel.roadLine[3].segment[i+1].offset)
				{
...
// added by Abbas
	if(receivedTrajectory_GPS)
	{
		...
		{	
			for(uint i=0; i<roadModel.roadLine[3].QtySegment.data-1; i++)
			{
				if(roadModel.roadLine[3].segment[i].offset <= roadModel_EH.roadLine[5].segment[m].offset 
					&& roadModel_EH.roadLine[5].segment[m].offset <= roadModel.roadLine[3].segment[i+1].offset)
				{
				  ...
				  // && (delta_offset < ZONE_DETECTION_OBSTACLE_CURVILIGNE)
				   ...
				   && (   (obstacleFusion.ObstacleFusion[id].existenceState.data == project_s_msgs::ObstacleFusion::CURRENT_TRACK)
					...
					if(EvitementObstacle) {
						if(index < project_s_msgs::RoadModel::SPEED_LIMIT_ARRAY_SIZE && obstacleFusion.ObstacleFusion[id].obstaclePosition.position.y < 0){  //-1.5){
					...
					if(index < project_s_msgs::RoadModel::SPEED_LIMIT_ARRAY_SIZE){
						...
						if(delta_offset < 1.5*Distance_darret_obligatoire){  
							speedCollisionAvoidance[0].offset = 0;
							speedCollisionAvoidance[0].speed  = 0;
							index=1;
						}	
						else if (!EvitementObstacle)
						{
							speedCollisionAvoidance[index].offset = delta_offset - DISTANCE_ARRET;
							speedCollisionAvoidance[index].speed  = 0;
							index++;
