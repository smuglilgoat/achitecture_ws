...
*/
			...
			if(amer_assigned){
				...
				if(matching_initial || ((abs(d_X) < 2.0) && (abs(d_Y) < 2.0))){
					...
					float dt = 0.08;//(current_time - last_time).toSec();
					last_time = current_time;

					#define CONSTANTE_TEMPS_AMER	10
					double K = exp(-dt/CONSTANTE_TEMPS_AMER);
K = 0.94;
...
//d_Y = 0;
				if(  (abs(d_Cap) < 20/180.0*M_PI) && (abs(Cap_EH_R) < 25/180.0*M_PI) && (abs(Cap_ME_R) < 25/180.0*M_PI)
				   &&(abs(d_Y) < 2.0)){
					...
					// MàJ du vecteur de correction, dans le ref Ego, avec filtrage
					// lissage de vth au 1er ordre
					double dt = 0.08;//(current_time - last_time).toSec();
					last_time = current_time;

					#define CONSTANTE_TEMPS_LINE	1
					double K = exp(-dt/CONSTANTE_TEMPS_LINE);
K = 0.97;
