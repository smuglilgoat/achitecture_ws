...
#define Y_DECALAGE_EVITEMENT		2  //1.5  m
#define X_DEBUT_CHANGEMENT_VOIE		15.0
#define X_FIN_CHANGEMENT_VOIE		5.0
...
if((ArretObstacle) && (EvitementObstacle)){
...
if(  (obstacleAssignment.ObstacleFusion[k].NbrOccupiedLane == 1) 
   &&(obstacleAssignment.ObstacleFusion[k].distanceCurviligne > -20.0) && (obstacleAssignment.ObstacleFusion[k].distanceCurviligne < 30.0)){  //< 20.0)){
	...
	// cas d'évitement d'obstacle systématiquement par la gauche, si l'obstacle est à droite de la voie
	for(uint i=0; i<trajectory_GPS.trajectoryLine.QtySegment.data; i++){
		...
		// - pour les points 15m avant et après l'obstacle,
		// - pour les points devant le véhicule jusqu'à 20m
		if((deltaY < 0.0) && (abs(d_X) < X_DEBUT_CHANGEMENT_VOIE) /*&& (trajectory_GPS.trajectoryLine.segment[i].offset < 20.0) */)
		{
			if(abs(d_X) < X_FIN_CHANGEMENT_VOIE)
				trajectory_GPS.trajectoryLine.segment[i].position.y += Y_DECALAGE_EVITEMENT;
			else
				trajectory_GPS.trajectoryLine.segment[i].position.y += (X_DEBUT_CHANGEMENT_VOIE - abs(d_X))/(X_DEBUT_CHANGEMENT_VOIE - X_FIN_CHANGEMENT_VOIE) * Y_DECALAGE_EVITEMENT;
		}
	}
}
}
