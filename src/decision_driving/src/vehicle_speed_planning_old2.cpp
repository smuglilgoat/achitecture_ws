...
int main(int argc, char** argv)
{
  	...
  	while(n.ok())
	{
		...
		if(roadModelReceived)
		{
			...
			//-----
			if(mission_manager_status.data == MISSION_STOP){
...
//}
			...
			#define	DECELERATION_MAX	+1.0   //  +0.5	// m/s2		signe +, on gère le signe negatif dans le code
			...
			for(int i = 0; i < project_s_msgs::RoadModel::SPEED_LIMIT_ARRAY_SIZE-1; i++){
				...
				while((index < project_s_msgs::RoadModel::SPEED_LIMIT_ARRAY_SIZE) && (delta_nbr > 0)){
					speed_reward_buffer[index] = speed_forward_buffer[index] = speedEnveloppe[i].speed;	// consigne théorique
					index++;
					delta_nbr--;
				}
			}
